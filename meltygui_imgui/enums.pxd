cdef extern from "imgui.h":
    ...
    ctypedef enum ImGuiConfigFlags_:
        ...
        ImGuiConfigFlags_NavEnableGamepad       # Master gamepad navigation enable flag. This is mostly to instruct your meltygui_imgui backend to fill io.NavInputs[]. Backend also needs to set ImGuiBackendFlags_HasGamepad.
        ...
        ImGuiConfigFlags_NoMouse                # Instruct meltygui_imgui to clear mouse position/buttons in NewFrame(). This allows ignoring the mouse information set by the backend.
        ImGuiConfigFlags_NoMouseCursorChange    # Instruct backend to not alter mouse cursor shape and visibility. Use if the backend cursor changes are interfering with yours and you don't want to use SetMouseCursor() to change mouse cursor. You may want to honor requests from meltygui_imgui by reading GetMouseCursor() yourself instead.
    ...
    ctypedef enum ImGuiDragDropFlags_:
        ...
        ImGuiDragDropFlags_SourceAllowNullID            # Allow items such as Text(), Image() that have no unique identifier to be used as drag source, by manufacturing a temporary identifier based on their window-relative position. This is extremely unusual within the dear meltygui_imgui ecosystem and so we made it explicit.
        ImGuiDragDropFlags_SourceExtern                 # External source (from outside of dear meltygui_imgui), won't attempt to read current item/window info. Will always return true. Only one Extern source can be active simultaneously.
    ...
    ctypedef enum ImGuiViewportFlags_:
        ...
        ImGuiViewportFlags_OwnedByApp               # Platform Window: is created/managed by the application (rather than a dear meltygui_imgui backend)
